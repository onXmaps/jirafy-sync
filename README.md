# Jirafy Sync
Sync Jira tickets and version based on a given changelog (ideally generated by [actions/jirafy-changelog](https://github.com/marketplace/actions/jirafy-changelog))
...
- I recommend letting [coltdorsey/jirafy-changelog](https://github.com/coltdorsey/jirafy-changelog) action generate your changelog. The changelog can be generated given there is already at least one github release in your repository.
...
- Finally you'll want to sync the github release with a jira version using [Jirafy Sync](https://github.com/coltdorsey/jirafy-sync#jirafy-sync). The github release version created from [actions/actions/create-release@latest](https://github.com/actions/create-release) will produce the latest release that this action will use to create the corresponding Jira project versions, as well as update the fix version of each story that was found in the given changelog.
...
jobs:
  release:
    ...
    steps:
      ...
      - name: Jirafy Sync
        uses: coltdorsey/jirafy-sync@v1.0.1
        with:
