...
async function setIssueProperties(issueId, issueUpdate) {
  const bodyData =
    typeof issueUpdate === 'string' ? issueUpdate : JSON.stringify(issueUpdate)
...
function createVersionAndUpdateFixVersions(changelog, version) {
  ...
  try {
    ...
    projects.forEach(async (project) => {
      console.log(
        '\x1b[32m%s\x1b[0m',
        `Attempting to create Jira version: ${version} in project: ${project}`
      )
      ...
      tickets.forEach(async (ticket) => {
        console.log(
          '\x1b[32m%s\x1b[0m',
          `Attempting to set fix version: ${version} for ticket: ${ticket}`
        )
